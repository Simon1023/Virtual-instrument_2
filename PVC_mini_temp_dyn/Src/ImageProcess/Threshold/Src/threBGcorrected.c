/* threshold BG-corrected method */

#include "../threBGcorrected.h"
#include "../threInterVariance.h"

int threBGcorrected(uc1D *image, int row, int col)
{
	int i, j, ii, jj, hf_row, hf_col, threshold;
	uc1D bgImg;
	i1D temp1, temp2, count1, count2;
	hf_row = row/2;
	hf_col = col/2;
	bgImg = uc1D_Initial(image->nr, image->nc);
	...
	*/
	...
	for (i=0; i<image->nr; i++)
	{
		...
		//(temp2->m[i][0]/count2->m[i][0]) is local mean
		bgImg.m[i*image->nc+0] = image->m[i*image->nc+0] - temp2.m[i*image->nc+0]/count2.m[i*image->nc+0];
	}
	...
	for (i=0; i<image->nr; i++){
		for (j=1; j <image->nc; j++){
			...
			bgImg.m[i*image->nc+j] = image->m[i*image->nc+j] - temp2.m[i*image->nc+j]/count2.m[i*image->nc+j];
		}
	}
			
	threshold = threInterVar(&bgImg);
